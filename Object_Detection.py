...
def predict(sess, image_file, heat_map):
    """
    ...
    #file_Name = image_file[0].filename.split("/")[-1]
    print('Found {} boxes'.format(len(out_boxes)))
    # Generate colors for drawing bounding boxes.
    colors = generate_colors(class_names)
    # Draw bounding boxes on the image file
    draw_boxes(image,heatMap, out_scores, out_boxes, out_classes, class_names, colors)
    # Save the predicted bounding box on the image
    #image.save(os.path.join("out", "test222.jpg"), quality=100)
    # Display the results in the notebook
    #output_image = scipy.misc.imread(os.path.join("out", file_Name))
    #imshow(output_image)
